...
// linear combination of (N-1) components vectors to use for the constrained forces
inline void vec_lin_comb_cmplx_coeff_matrix_new(cmplx *res, cmplx const *const v, cmplx const *const w, cmplx const a, cmplx const b, cmplx const *const A, cmplx const *const B)
{
#ifdef __INTEL_COMPILER
	__assume_aligned(&(res), DOUBLE_ALIGN);
	__assume_aligned(&(v), DOUBLE_ALIGN);
...
inline cmplx vec_scalar_prod_matrix_new(cmplx const *const v, cmplx const *const w, cmplx const *const A)
{
#ifdef __INTEL_COMPILER
	__assume_aligned(&(v), DOUBLE_ALIGN);
	__assume_aligned(&(w), DOUBLE_ALIGN);
