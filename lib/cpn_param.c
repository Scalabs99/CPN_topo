...
void read_input(char const *const input_file_name, CPN_Param *param)
{
	...
	else
	{
		while (end == 1) // slide the file
		{
			...
			else if (strncmp(str, "topoGrad_file", 13) == 0)
			{
				...
				strcpy(param->d_topoGrad_file, temp_str);
			}
			...
			else if (strncmp(str, "argP_file", 9) == 0)
			{
				...
				strcpy(param->d_argP_file, temp_str);
			}
			...
			temp_i = getc(input_fp);
			if (temp_i == EOF)
				end = 0; // if end of file is reached, end=0 and exit the while loop
			else
				ungetc(temp_i, input_fp);
...
void init_derived_constants(CPN_Param *param)
{
	...
	param->d_volume = 1;
	for (mu = 0; mu < 2; mu++)
		(param->d_volume) *= (param->d_size[mu]);
}
	...
	*dataf = fopen(param->d_data_file, "r");
	...
	else // file doesn't exist
	{
		...
		*dataf = fopen(param->d_data_file, "w");
		...
		for (mu = 0; mu < 2; mu++)
			fprintf(*dataf, "%d ", param->d_size[mu]);
		fprintf(*dataf, "\n");
	}
	fflush(*dataf);
}
	...
	*topofilep = fopen(param->d_topo_file, "r");
	...
	else
	{
		...
		*topofilep = fopen(param->d_topo_file, "w");
		...
		for (mu = 0; mu < 2; mu++)
			fprintf(*topofilep, "%d ", param->d_size[mu]);
		fprintf(*topofilep, "\n");
	}
	fflush(*topofilep);
}
...
void init_topograd_file(FILE **topogradf, CPN_Param const *const param)
{
	*topogradf = fopen(param->d_topoGrad_file, "r");
	if (*topogradf != NULL) // file exists
	{
		fclose(*topogradf);
		*topogradf = fopen(param->d_topoGrad_file, "a");
	}
	else // file doesn't exist
	{
		int mu;
		*topogradf = fopen(param->d_topoGrad_file, "w");
		fprintf(*topogradf, "# %f ", param->d_beta);
		for (mu = 0; mu < 2; mu++)
			fprintf(*topogradf, "%d ", param->d_size[mu]);
		fprintf(*topogradf, "\n");
	}
	fflush(*topogradf);
}
	...
	*coolf = fopen(param->d_cool_file, "r");
	...
	else // file doesn't exist
	{
		...
		*coolf = fopen(param->d_cool_file, "w");
		...
		for (mu = 0; mu < 2; mu++)
			fprintf(*coolf, "%d ", param->d_size[mu]);
		fprintf(*coolf, "\n");
	}
	fflush(*coolf);
}
	...
	*gradf = fopen(param->d_grad_file, "r");
	...
	else // file doesn't exist
	{
		...
		*gradf = fopen(param->d_grad_file, "w");
		...
		for (mu = 0; mu < 2; mu++)
			fprintf(*gradf, "%d ", param->d_size[mu]);
		fprintf(*gradf, "\n");
	}
	fflush(*gradf);
}
...
// initialize argP file for cooling
void init_argP_file(FILE **argPf, CPN_Param const *const param)
{
	*argPf = fopen(param->d_argP_file, "r");
	if (*argPf != NULL) // file exists
	{
		fclose(*argPf);
		*argPf = fopen(param->d_argP_file, "a");
	}
	else // file doesn't exist
	{
		int mu;
		*argPf = fopen(param->d_argP_file, "w");
		fprintf(*argPf, "# %f ", param->d_beta);
		for (mu = 0; mu < 2; mu++)
			fprintf(*argPf, "%d ", param->d_size[mu]);
		fprintf(*argPf, "\n");
	}
	fflush(*argPf);
}
								  ...
								  clock_t const start_time, clock_t const finish_time)
{
	...
	else
	{
		...
		if (param->d_N_replica_pt > 1)
			fprintf(fp, "Number of replicas used for parallel tempering: %d\n", param->d_N_replica_pt);
		if (param->d_start == 0)
			fprintf(fp, "Simulation started from random cold conf\n");
		if (param->d_start == 1)
			fprintf(fp, "Simulation started from random hot conf\n");
		if (param->d_start == 2)
			fprintf(fp, "Simulation started from stored conf read from file %s", param->d_conf_file);
		if (param->d_N_replica_pt > 1)
			fprintf(fp, "_replica_*\n");
		else
			fprintf(fp, "\n");
		...
		if (param->d_L_defect > 0)
			fprintf(fp, "Defect size: %d\n", param->d_L_defect);
		if (param->d_N_hierarc_levels > 0)
		{
		...
		if (param->d_N_replica_pt == 1)
			fprintf(fp, "Number of standard local updating steps: %d\n", param->d_MC_step);
		else
			fprintf(fp, "Number of parallel tempering steps: %d\n", param->d_MC_step);
		...
		if (param->d_rng_start == 0)
			fprintf(fp, "Rng initializated from seed %ld\n", param->d_seed);
		if (param->d_rng_start == 1)
			fprintf(fp, "Rng initializated from last rng state read from file %s (seed = %ld)\n", param->d_rng_file, param->d_seed);
		fprintf(fp, "\n");
											   ...
											   clock_t const start_time, clock_t const finish_time)
{
	...
	else
	{
		...
		if (param->d_N_replica_pt == 1)
			fprintf(fp, "Number of standard local stochastic updating steps: %d\n", param->d_MC_step);
		else
			fprintf(fp, "Number of multicanonic stochastic parallel tempering steps:    %d\n", param->d_MC_step);
