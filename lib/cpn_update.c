...
{
	int j;
	for (j = 0; j < param->d_num_micro; j++)
		microcanonic_sweep_rectangle(conf, geo, param, most_update);
...
void update_with_defect(CPN_Conf *conf, Geometry const *const geo, CPN_Param const *const param, RNG_Param *rng_state)
{
	int j;
	for (j = 0; j < param->d_num_micro; j++)
		microcanonic_sweep_lattice(conf, geo, param);
...
cmplx force_U(CPN_Conf const *const conf, Geometry const *const geo, CPN_Param const *const param, long const i, int const mu)
{
	...
	a = vector_scalar_product_matrix(conf->z[geo->up[i][mu]], conf->z[i], conf->M1[i][mu]);
	b = vector_scalar_product_matrix(conf->z[geo->up[geo->up[i][mu]][mu]], conf->z[i], conf->M2[i][mu]);
	c = vector_scalar_product_matrix(conf->z[geo->up[i][mu]], conf->z[geo->dn[i][mu]], conf->M2[geo->dn[i][mu]][mu]);
...
double F_theta(CPN_Conf const *const conf, Geometry const *const geo, long const i, int const mu)
{
	...
	a *= conf->C[i][mu] * conj(conf->U[i][mu]);
...
void F_z_constrained(CPN_Conf const *const conf, Geometry const *const geo, long i, cmplx *F_z)
{
	...
	{	
		...
		coeff2 = conf->C[i][mu] * conf->U[i][mu];
		// aux = coeff1 * z(i-mu) + coeff2 * z(i+mu)
		vec_lin_comb_cmplx_coeff_matrix_new(aux, conf->z[geo->dn[i][mu]], conf->z[geo->up[i][mu]], coeff1, coeff2, conf->M1[geo->dn[i][mu]][mu], conf->M1[i][mu]);
		vector_sum(x1, aux); // x1 += aux
...
void microcanonic_sweep_rectangle(CPN_Conf *conf, Geometry const *const geo, CPN_Param const *const param, Rectangle const *const most_update)
{
	...
	for (i = 0; i < (most_update->d_vol_rect); i++)
	{
		...
		for (mu = 0; mu < 2; mu++)
			microcanonic_single_link_U(conf, geo, param, n, mu);
...
void microcanonic_sweep_lattice(CPN_Conf *conf, Geometry const *const geo, CPN_Param const *const param)
{
	...
	{
		for (mu = 0; mu < 2; mu++)
			microcanonic_single_link_U(conf, geo, param, i, mu);
...
void microcanonic_single_link_U(CPN_Conf *conf, Geometry const *const geo, CPN_Param const *const param, long const i, int const mu)
{
	...
	if (mod_F_U != 0)
		conf->U[i][mu] += (2.0 * p * F_U / mod_F_U); // U_new = 2 p U_old F_U/|F_U|^2 - U_old if F_U!=0
}
...
void microcanonic_single_site_z(CPN_Conf *conf, Geometry const *const geo, long const i)
{
	...
	if (mod_F_z != 0)
		vector_linear_combination_real_coeff(conf->z[i], F_z, -1.0, b); // z_new = -z_old + b F_z if F_z!=0
	else
		vector_times_real_const(conf->z[i], -1.0); // z_new = -z_old if F_z=0
}
...
void overheatbath_sweep_rectangle(CPN_Conf *conf, Geometry const *const geo, CPN_Param const *const param, Rectangle const *const most_update, RNG_Param *rng_state)
{
	...
	for (i = 0; i < most_update->d_vol_rect; i++)
	{
		n = most_update->rect_sites[i];
		for (mu = 0; mu < 2; mu++)
			overheatbath_single_link_U(conf, geo, param, n, mu, rng_state);
...
void overheatbath_sweep_lattice(CPN_Conf *conf, Geometry const *const geo, CPN_Param const *const param, RNG_Param *rng_state)
{
	...
	{
		for (mu = 0; mu < 2; mu++)
			overheatbath_single_link_U(conf, geo, param, i, mu, rng_state);
...
void overheatbath_single_link_U(CPN_Conf *conf, Geometry const *const geo, CPN_Param const *const param, long const i, int const mu, RNG_Param *rng_state)
{
	...
	if (cos_theta_old > (1.0 - 1000.0 * NUM_EPS) || mod_F_U == 0)
		cos_theta_old = 1.0 - (1000.0 * NUM_EPS);		   // avoid cos(theta_old) too close to 1
	sin_2_theta_old = 1.0 - cos_theta_old * cos_theta_old; // this is sin^2(theta_old)
	if (sin_2_theta_old < NUM_EPS || mod_F_U == 0)
		sin_2_theta_old = NUM_EPS; // avoid sin(theta_old) too close to 0
	...
	if (mod_F_U != 0)
		conf->U[i][mu] += b1 * F_U;				 // U_new = -b U_old + b1 F_U if F_U!=0
...
void overheatbath_single_site_z(CPN_Conf *conf, Geometry const *const geo, CPN_Param const *const param, long const i, RNG_Param *rng_state)
{
	...
	if (sin_2_theta_old < NUM_EPS || mod_F_z == 0)
		sin_2_theta_old = NUM_EPS; // avoid sin(theta_old) too close to 0
	if (sin_2_theta_new < NUM_EPS)
		sin_2_theta_new = NUM_EPS;				 // avoid sin(theta_new) too close to 0
	b = sqrt(sin_2_theta_new / sin_2_theta_old); // sin(theta_new)/sin(theta_old)
	b1 = (cos_theta_new + cos_theta_old * b) / mod_F_z;

	if (mod_F_z != 0)
		vector_linear_combination_real_coeff(conf->z[i], F_z, -b, b1); // z_new = -b z_old + b1 F_z if F_z!=0
	else
		vector_times_real_const(conf->z[i], -b); // z_new = -b z_old if F_z=0
...
void conf_translation(CPN_Conf *conf, Geometry const *const geo, CPN_Param const *const param, CPN_Conf *aux_conf, RNG_Param *rng_state)
{
	...
	for (i = 0; i < 2; i++)
	{
		if ((aux > ((double)i)) && (aux < ((double)(i + 1))))
			dir = i;
	}
	...
	for (r = 0; r < param->d_volume; r++)
	{
		...
		for (mu = 0; mu < 2; mu++)
			conf->U[r][mu] = aux_conf->U[geo->up[r][dir]][mu];
	}
}
