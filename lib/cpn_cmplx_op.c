...
void vec_lin_comb_cmplx_coeff_matrix_new(cmplx *res, cmplx const *const v, cmplx const *const w, cmplx const a, cmplx const b, cmplx const *const A, cmplx const *const B);
void vector_linear_combination_real_coeff(cmplx *v, cmplx const *const w, double const a, double const b);
void vec_lin_comb_3_real_coeff(cmplx *v, cmplx const *const w, cmplx const *const u, double const a, double const b, double const c);
void vec_lin_comb_4_real_coeff(cmplx *v, cmplx const *const w, cmplx const *const u, cmplx const *const z, double const a, double const b, double const c, double const d);
...
void write_cmplx_num_on_file_binary(FILE *fp, cmplx const *const a)
{
	...
	if (is_little_endian() == 0)
		write_cmplx_num_on_file_binary_little_endian(fp, a);
	// if machine is big endian, write without bytes swap
	else
		write_cmplx_num_on_file_binary_big_endian(fp, a);
}
...
void write_cmplx_vec_on_file_binary(FILE *fp, cmplx const *const v)
{
	int k;
	for (k = 0; k < N; k++)
		write_cmplx_num_on_file_binary(fp, &(v[k]));
}
...
void read_cmplx_num_from_file_binary(FILE *fp, cmplx *a)
{
	// if machine is little endian, write after bytes swap
	if (is_little_endian() == 0)
		read_cmplx_num_from_file_binary_little_endian(fp, a);
	// if machine is big endian, write without bytes swap
	else
		read_cmplx_num_from_file_binary_big_endian(fp, a);
}
...
void read_cmplx_vec_from_file_binary(FILE *fp, cmplx *v)
{
	int k;
	for (k = 0; k < N; k++)
		read_cmplx_num_from_file_binary(fp, &(v[k]));
}
